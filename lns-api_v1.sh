...
# ============================================================================
# Version: 3.0 - Smart Latest Data Management with Auto-Cleanup
# Features:
#   - Exports to timestamped archives (for history)
#   - Maintains "current" directory with latest data
#   - API always serves latest data automatically
#   - Auto-cleanup of old exports (keeps last 10)
#   - Cron-friendly (run every 15 minutes)
#   - Auto-restart API server with new data
# ============================================================================
...
# CREATE BASE_DIR FIRST - ADD THIS LINE HERE
mkdir -p "$BASE_DIR"

# Generate or reuse API token (NOW this will work)
...
# Create subdirectories (BASE_DIR already created above)
mkdir -p "$ARCHIVE_DIR"
mkdir -p "$CURRENT_DIR"
...
echo "=========================================="
echo "IoTKinect Smart Export + API v3.0"
echo "=========================================="
...
echo -e "${BLUE}API:${NC} http://$(hostname -I | awk '{print $1}'):$API_PORT"
...
echo -e "${YELLOW}Test API:${NC} curl http://$(hostname -I | awk '{print $1}'):$API_PORT/health"
echo ""
